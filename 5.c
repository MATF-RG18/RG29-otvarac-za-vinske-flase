...
float rastojanje_flase_i_otvaraca = 5;
...
/*Izabran je prvi otvarac*/
	...
	{
/*Izabran je prvi otvarac*/
		if(fabs(spust_spirale + rastojanje_flase_i_otvaraca + 6) <= 0.5 && fabs(spust_tela + rastojanje_flase_i_otvaraca + 6) <= 0.5 && spust_pampura == 0)
		{
/*Prvo se i telo i spirala otvaraca krecu ka flasi istovremeno. Kada dodju do flase, spirala se zaustavlja, a telo se spusta jos malo. Zato stavljamo korak spirale na 0*/
		...
		else if(fabs(spust_spirale + rastojanje_flase_i_otvaraca + 6) <= 0.5 && fabs(spust_tela + rastojanje_flase_i_otvaraca + duzina_pampura + 6) <= 0.5)
		{
...
Postavljamo korak za ugao rotacije i korak spustanja spirale na odgovarajuce vrednosti, a korak tela stavljamo na 0*/
		...
		else if(fabs(spust_spirale + rastojanje_flase_i_otvaraca + duzina_pampura + 6) <= 1.0/18 && fabs(spust_pampura - 1.5*duzina_pampura) > 1.0/18)
		{
...
/*Kada je pampur izasao iz flase i podigao dovoljno uz spiralu zaustaviti kretanje*/
		...
		else if(fabs(spust_spirale - rastojanje_flase_i_otvaraca + duzina_pampura) <= 0.5 && fabs(spust_tela - rastojanje_flase_i_otvaraca + duzina_pampura) <= 0.5)
		{printf("%f %f\n", spust_spirale, spust_tela);
			korak_spirale = 0;
			korak_pampura = 0;
			korak_tela = 0;
		}
...
/*Translira se na gore ako je korak pozitivan odnosno ako je otvarac krenuo da ga vadi (radimo disjunkciju da bi vazilo za obe animacije)*/
		if(korak >= 0 || spust_pampura > 0)
		{
			glTranslatef(0, duzina_pampura + spust_pampura, 0);
		}
		glRotatef(90, 1, 0, 0);
		gluCylinder(quad, 1.25, 1.25, duzina_pampura, 20, 10);
		glRotatef(-90, 1, 0, 0);
		if(korak >= 0 || spust_pampura > 0)
		{
			glTranslatef(0, -(duzina_pampura + spust_pampura), 0);
...
/*Pravi se valjak koji predstavlja unutrasnji deo grlica flase, iznad tog sfernog dela; rotira se do adekvatnog polozaja i translira iznad polusfere*/
		glTranslatef(0, 12, 0);
		glRotatef(90, 1, 0, 0);
		gluCylinder(quad, 1.25, 1.25, 8, 40, 40);
		glRotatef(-90, 1, 0, 0);
		glTranslatef(0, -12, 0);
...
/*Pravi se valjak koji predstavlja spoljasnji deo grlica flase, iznad tog sfernog dela; rotira se do adekvatnog polozaja i translira iznad polusfere*/
		glTranslatef(0, 12, 0);
		glRotatef(90, 1, 0, 0);
		gluCylinder(quad, 1.5, 1.5, 8, 40, 40);
		glRotatef(-90, 1, 0, 0);
		glTranslatef(0, -12, 0);
