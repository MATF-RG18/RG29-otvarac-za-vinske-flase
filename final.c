...
/*Promenljive koje se koriste u tajmeru (korakY, korak_ugao) i u funkcijama za isrtavanje otvaraca (korakY, korakUgao)
korak_ugao- za toliko se menja ugao rotacije otvaraca u svakom pozivu tajmera
korakUgao- koristi se za pravljenje spirale u otvarac1 i otvarac2
korakY- koristi se za pravljenje spirale u otvarac1 i otvarac2 i za poredjenje float brojeva u tajmeru*/
float korakY = 1.0/180, korakUgao = PI/180, korak_ugao = 20;
float korak = -1.0/9;

/*ugao- ugao za rotiranje otvaraca, menja se u tajmeru*/
float ugao = 0;

/*duzina_pampura- koristi se u tajmeru i u funkciji za iscrtavanje flase za pravljenje pampura*/
float duzina_pampura = 4;

/*Promenljive koje se menjaju u tajmeru, a koriste funkcijama za iscrtavanje flase(pampura) i otvaraca da definisu kretanje tih objekata:
...
ugao_pogleda- promenljiva koja se menja u on_keybord funkciji, a koristi u on_display da pomera kameru*/
float rastojanje_flase_i_otvaraca = 5, ugao_pogleda = 0;
...
/*Postavljamo da je animacija neaktivna*/
    timer_active = 0;
...
/*Postavlja se boja ekrana na boju breskve :3 */
...
/*Zavrsava se program*/
	...
	case 'u':
	case 'U':
/*Ugao pogleda se menja za plus 10 stepeni*/
		ugao_pogleda += PI/18;
		if(ugao_pogleda >= 2*PI)
		{
			ugao_pogleda -= 2*PI;
		}
    	glutPostRedisplay();
	...
	case 'L':
	case 'l':
/*Ugao pogleda se menja za minus 10 stepeni*/
		ugao_pogleda -= PI/18;
		if(ugao_pogleda <= -2*PI)
		{
			ugao_pogleda += 2*PI;
...
/*Pokrece se simulacija (g->GO)*/
		...
		{
            timer_active = 1;
            glutTimerFunc(5, on_timer, timer_id);
        }
        break;
...
/*menjamo spust odnosno pomeraj otvaraca, a kasnije i pampura, duz y-osu*/
	...
	{
/*Izabran je drugi otvarac*/
		if(fabs(spust_spirale + rastojanje_flase_i_otvaraca) <= 0.25 && fabs(spust_tela + rastojanje_flase_i_otvaraca) <= 0.25 && spust_pampura == 0)
		{
/*Prvo se i telo i spirala otvaraca krecu ka flasi istovremeno. Kada dodju do flase, spirala se zaustavlja, a telo se spusta jos malo. Zato stavljamo korak spirale na 0*/
		...
		else if(fabs(spust_spirale + rastojanje_flase_i_otvaraca) <= 0.25 && fabs(spust_tela + rastojanje_flase_i_otvaraca + duzina_pampura) <= 0.25)
		{
...
Postavljamo korak za ugao rotacije i korak spustanja spirale na odgovarajuce vrednosti, a korak tela stavljamo na 0*/
		...
		else if(fabs(spust_spirale + rastojanje_flase_i_otvaraca + duzina_pampura) <= 1.0/18 && fabs(spust_pampura - 1.5*duzina_pampura) > 1.0/18)
		{
...
Postavljamo korak spirale na nula i korak podizanja pampura na odgovarajucu vrednost.*/
		...
		{
/*Kada je pampur izasao iz flase i podigao dovoljno uz spiralu, podizemo ceo otvarac zajedno sa pampurom*/
			korak_ugao = 0;
			korak_pampura = 0.5;
			korak_spirale = 0.5;
			korak_tela = 0.5;
		}
		else if(fabs(spust_spirale - rastojanje_flase_i_otvaraca + duzina_pampura) <= 0.25 && fabs(spust_tela - rastojanje_flase_i_otvaraca + duzina_pampura) <= 0.25)
		{
/*Kada smo dovoljno podigli otvarac, zaustavljamo kretanje*/
			korak_spirale = 0;
			korak_pampura = 0;
			korak_tela = 0;
		}
...
/*Postavlja se svetlo i polozaj kamere*/
    ...
    glLoadIdentity();
/*Polozaj kamere je postavljen tako da moze da se menja u zavisnosti od ugla koji je izracunat u on_keyboard funkciji*/
    gluLookAt(0, 60*sin(ugao_pogleda), 60*cos(ugao_pogleda), 0, 0, 0, 0, 1, 0);
...
/*Promenljiva neophodna za valjke koji se prave u funkcijama za isrtavanje*/
	GLUquadric* quad = gluNewQuadric();

/*duzina_otvaraca- koristi se za transliranje otvaraca iznad xz-ravni; u zavisnosti koji otvarac je izabran
duzina je razlicita (duzine su skracene za 1.5 jer se oba otvaraca toliko vec nalaze iznad xz-ravni)*/
	float duzina_otvaraca;

/*Transliramo sve komponente uz y-osu*/
	glTranslatef(0, 8, 0);
...
Ako je timer_id = 0, zovemo prvi otvarac*/
	if(timer_id)
	{
/*Flasu iscrtavamo i spustamo za pola rastojanja izmedju flase i otvaraca*/
		glTranslatef(0, -rastojanje_flase_i_otvaraca/2.0, 0);
		flasa(quad);
		glTranslatef(0, rastojanje_flase_i_otvaraca/2.0, 0);

/*Postavljamo duzinu otvaraca na 22 (prava duzina je 23.5 ali pola drske se vec nalazi iznad xz-ravni, pa nam to ne treba) i
iscrtavamo i transliramo otvarac za svoju duzinu (da bude ceo iznad xz-ravni) i za pola rastojanja izmedju flase i otvaraca*/
		duzina_otvaraca = 22;
		glTranslatef(0, duzina_otvaraca+rastojanje_flase_i_otvaraca/2.0, 0);
		otvarac2(quad);
		glTranslatef(0, -duzina_otvaraca-rastojanje_flase_i_otvaraca/2.0, 0);
	}
	else
...
/*Postavljamo duzinu otvaraca na 16.5 (prava duzina je 18 ali pola drske se vec nalazi iznad xz-ravni,
pa nam to ne treba) i iscrtavamo i transliramo otvarac za svoju duzinu (da bude ceo iznad xz-ravni)*/
		duzina_otvaraca = 16.5;
		glTranslatef(0, duzina_otvaraca, 0);
		otvarac1(quad);
		glTranslatef(0, -duzina_otvaraca, 0);
	}

/*Vracamao ostale objekte nazad niz y-osu*/
	glTranslatef(0, -8, 0);
...
void flasa(GLUquadric* quad)
{
/*Postavljaju se velicine razlicitih delova flase; ima ih duplo zato sto jedni opisuju unutrasnji sloj stakla, a drugi spoljasnji*/
	float duzina_grlica0 = 8.4, duzina_grlica1 = 8.2, poluprecnik_grlica0 = 1.5, poluprecnik_grlica1 = 1.75, poluprecnik_sfere0 = 5.5, poluprecnik_sfere1 = 5.7, duzina_valjka0 = 22, duzina_valjka1 = 22.2;
...
/*Translira se na gore ako je korak ili spust_pampura pozitivan odnosno ako je otvarac krenuo da ga vadi (radimo if da bi vazilo za obe animacije)*/
		if(timer_id == 1 && spust_pampura > 0)
			glTranslatef(0, duzina_pampura + spust_pampura, 0);
		else if(korak >= 0 && timer_id == 0)
			...
			glBegin(GL_POLYGON);
				...
				{
					glNormal3f(-1, 0, 0);
					glVertex3f(poluprecnik_grlica0*cos(ugao_krug), 0, poluprecnik_grlica0*sin(ugao_krug));
				}
			glEnd();

			glRotatef(90, 1, 0, 0);
				gluCylinder(quad, poluprecnik_grlica0, poluprecnik_grlica0, duzina_pampura, 20, 10);
			glRotatef(-90, 1, 0, 0);

			glTranslatef(0, -duzina_pampura, 0);
				glBegin(GL_POLYGON);
				for(ugao_krug = 0; ugao_krug <= 2*PI + PI/20; ugao_krug += PI/20)
				{
					glNormal3f(-1, 0, 0);
...
/*Pravi se unutrasnji deo polusfere; translira se za duzinu grlica*/
		glTranslatef(0, -duzina_grlica0-poluprecnik_sfere0+0.3, 0);

			glEnable(GL_CLIP_PLANE0);
				glClipPlane(GL_CLIP_PLANE0, clip_plane);
				glutSolidSphere(poluprecnik_sfere0, 40, 40);
	   		glDisable(GL_CLIP_PLANE0);

/*Pravi se valjak koji predstavlja unutrasnji deo flase, ispod tog sfernog dela; rotira se do adekvatnog polozaja*/
			glRotatef(90, 1, 0, 0);
				gluCylinder(quad, poluprecnik_sfere0, poluprecnik_sfere0, duzina_valjka0, 40, 40);
			glRotatef(-90, 1, 0, 0);

/*Pravi se unutrasnji deo polusfere koja cini dno*/
			glTranslatef(0, -duzina_valjka0, 0);
				glEnable(GL_CLIP_PLANE0);
					glClipPlane(GL_CLIP_PLANE0, clip_plane);
					glutSolidSphere(poluprecnik_sfere0, 40, 40);
		   		glDisable(GL_CLIP_PLANE0);
...
/*Pravi se valjak koji predstavlja unutrasnji deo grlica flase, iznad tog sfernog dela; rotira se do adekvatnog polozaja i translira iznad polusfere*/
		...
		glRotatef(90, 1, 0, 0);
			gluCylinder(quad, poluprecnik_grlica0, poluprecnik_grlica0, duzina_grlica0, 40, 40);
		glRotatef(-90, 1, 0, 0);
...
/*Pravi se spoljasnji deo polusfere koja */
			glEnable(GL_CLIP_PLANE0);
				glClipPlane(GL_CLIP_PLANE0, clip_plane);
				glutSolidSphere(poluprecnik_sfere1, 40, 40);
	   		glDisable(GL_CLIP_PLANE0);

/*Pravi se valjak koji predstavlja spoljasnji deo flase, ispod tog sfernog dela; rotira se do adekvatnog polozaja*/
			glRotatef(90, 1, 0, 0);
				gluCylinder(quad, poluprecnik_sfere1, poluprecnik_sfere1, duzina_valjka1, 40, 40);
			glRotatef(-90, 1, 0, 0);

			glTranslatef(0, -duzina_valjka1, 0);
/*Pravi se spoljasnji deo polusfere koja cini dno*/
			glEnable(GL_CLIP_PLANE0);
				glClipPlane(GL_CLIP_PLANE0, clip_plane);
				glutSolidSphere(poluprecnik_sfere1, 40, 40);
	   		glDisable(GL_CLIP_PLANE0);
...
/*Pravi se valjak koji predstavlja spoljasnji deo grlica flase, iznad tog sfernog dela; rotira se do adekvatnog polozaja i translira iznad polusfere*/
		...
		glRotatef(90, 1, 0, 0);
			gluCylinder(quad, poluprecnik_grlica1, poluprecnik_grlica1, duzina_grlica1, 40, 40);
		glRotatef(-90, 1, 0, 0);
		...
		glBegin(GL_TRIANGLE_STRIP);
			...
			{
				glNormal3f(-1, 0, 0);
				glVertex3f(poluprecnik_grlica1*cos(ugao_krug), 0, poluprecnik_grlica1*sin(ugao_krug));
				glVertex3f(poluprecnik_grlica0*cos(ugao_krug), 0, poluprecnik_grlica0*sin(ugao_krug));
			}
		glEnd();
glPopMatrix();
...
void otvarac1(GLUquadric* quad)
{
	/*Postavlja se materijal za drveni deo odnosno drsku otvaraca*/
...
/*Prvi objekat (drska) se translira na dole po y-osi za spust koji se menja tajmerom i rotira za ugao koji se menja tajmerom*/
...
njihovi centri kad se povezu prave spiralu poluprecnika = precnik spirale i spusta po y-osi korakY. Tacke tih krugova povezujemo sa GL_TRIANGLE_STRIP*/
		glBegin(GL_TRIANGLE_STRIP);
		for(ugao_spirale = 0; ugao_spirale <= 7*krug + korakUgao; ugao_spirale += korakUgao)
		{
